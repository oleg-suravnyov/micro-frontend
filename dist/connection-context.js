...
const Connection = () => {
  ...
  connection.cleanConfiguration = () => {
    ...
    connection.ethereum.removeListener("accountsChanged", handleAccountsChanged);
    connection.ethereum.removeListener("chainChanged", handleChainChanged);
  };
  connection.connect = () => {
    connection.ethereum.request({
      method: "eth_requestAccounts"
    }).then(accounts => handleAccountsChanged(accounts)).catch(e => console.log("[ConnectionContext.handleAccountsChanged]", "error", e));
    connection.ethereum.request({
      method: "eth_chainId"
  ...
  connection.getBalance = async () => {
    ...
    return BigNumber.from(await connection.ethereum.request({
  ...
  connection.signLogin = async () => {
    try {
      ...
      console.log("[ConnectionContext.signLogin]", "timestamp", timestamp, "signature", signature);
      return {
        timestamp,
        signature
      };
  ...
  const handleAccountsChanged = accounts => {
    console.log("[ConnectionContext.handleAccountsChanged]", "wallet", accounts[0]);
...
export const ConnectionContext = /*#__PURE__*/React.createContext();
