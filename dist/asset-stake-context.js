...
const AssetStake = address => {
  ...
  assetStake.connect = connection => {
    ...
    assetStake.setContract(new ethers.Contract(assetStake.address, assetStakeABI, provider).connect(provider.getSigner()));
  ...
  assetStake.getBalance = async () => {
    ...
    try {
      const getStakeSize = await assetStake.contract.getStakesSize(assetStake.connection.wallet);
      console.log("[AssetStakeContext.getBalance]", "getStakeSize", getStakeSize);
...
export const AssetStakeContext = /*#__PURE__*/React.createContext();
export const AssetStakeProvider = params => {
  const assetStake = AssetStake(params.address);
  return /*#__PURE__*/React.createElement(AssetStakeContext.Provider, {
    value: assetStake
  }, params.children);
};