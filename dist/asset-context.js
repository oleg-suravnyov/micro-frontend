...
const Asset = address => {
  ...
  asset.connect = connection => {
    ...
    asset.setContract(new ethers.Contract(asset.address, assetABI, provider).connect(provider.getSigner()));
  ...
  asset.getCurrency = async () => {
    ...
    try {
      const getCurrency = await asset.contract.getCurrency(asset.defaultCurrency);
...
export const AssetContext = /*#__PURE__*/React.createContext();
